...
// 声明外部 UART 句柄
extern UART_HandleTypeDef huart2;  // 右电机
extern UART_HandleTypeDef huart3;  // 左电机
...
void Emm_V5_Read_Sys_Params_ZUO(uint8_t addr, SysParams_t s)
{
  ...
  // 发送命令（使用huart3）
  HAL_UART_Transmit(&huart3, cmd, i, HAL_MAX_DELAY);
}
...
void Emm_V5_Read_Sys_Params_YOU(uint8_t addr, SysParams_t s)
{
  ...
  // 发送命令（使用huart2）
  HAL_UART_Transmit(&huart2, cmd, i, HAL_MAX_DELAY);
}
...
void Emm_V5_Pos_Control_YOU(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF)
{
  ...
  // 发送命令（使用huart2）
  HAL_UART_Transmit(&huart2, cmd, 13, HAL_MAX_DELAY);
}
...
void Emm_V5_Stop_Now_YOU(uint8_t addr, bool snF)
{
  ...
  // 发送命令（使用huart2）
  HAL_UART_Transmit(&huart2, cmd, 5, HAL_MAX_DELAY);
}
/**
  * @brief    多机同步运动
  * @param    addr  ：电机地址
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void Emm_V5_Origin_Set_O_YOU(uint8_t addr, bool svF)
{
  uint8_t cmd[16] = {0};
