...
if (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE) != RESET) {
    ...
    // 1. 获取当前接收位置
    uint16_t len = UART_RX_BUFFER_SIZE - huart2.RxXferCount;
    
    // 2. 立即重置接收计数器和状态
    huart2.RxXferCount = UART_RX_BUFFER_SIZE;
...
if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE) != RESET) {
    ...
    // 1. 获取当前接收位置
    uint16_t len = UART_RX_BUFFER_SIZE - huart3.RxXferCount;
    
    // 2. 立即重置接收计数器和状态
    huart3.RxXferCount = UART_RX_BUFFER_SIZE;
...
void USART6_IRQHandler(void)
{
  /* USER CODE BEGIN USART6_IRQn 0 */
    ...
    // 1. 获取当前接收位置
    uint16_t len = UART_RX_BUFFER_SIZE - huart6.RxXferCount;
    
    // 2. 立即重置接收计数器和状态
    huart6.RxXferCount = UART_RX_BUFFER_SIZE;
    ...
    if (len > 0 && len <= UART_RX_BUFFER_SIZE) {
        ...
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if (uart6_rx_sem != NULL) {
            xSemaphoreGiveFromISR(uart6_rx_sem, &xHigherPriorityTaskWoken);
        }
